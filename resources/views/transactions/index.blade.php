                ...
                <table class="table-auto w-full">
                    <thead>
                        <tr>
                            ...
                            <th class="border px-6 py-4">USERS</th>
                        ...
                        @forelse ($transactions as $item)
                            <tr>
                                ...
                                <td class="border px-6 py-4 text-center">
                                    <a href="{{ route('transactions.show', $item->id) }}" class="inline-block bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 mx-2 rounded">View
                                    </a>
                                    <form action="{{ route('transactions.destroy', $item->id) }}" method="POST" class="inline-block">
                                        {!! method_field('delete') .csrf_field() !!}
                                        <button type="submit" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 mx-4 rounded">Delete</button>
                                    </form>
                                </td>
                            </tr>
                        @empty
